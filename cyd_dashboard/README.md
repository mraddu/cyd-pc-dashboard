...
- **HOME**: CPU ring gauge (+ temp), RAM ring gauge, GPU ring gauge (+ temp), network summary
- **Tap CPU** -> per-core load bars, frequency, temperature
- **Tap RAM** -> used/total, swap, top 3 memory-hungry processes
- **Tap GPU** -> utilization gauge, temperature
...
2. Run it:
   ...
   ```
3. The status dot on the CYD (top-right of the title bar) turns from red
...
3. In LibreHardwareMonitor's menu: **Options > Remote Web Server > Run**.
...
4. Leave LibreHardwareMonitor running in the background (minimizing is fine)
5. `pip install -r requirements.txt` again to pick up the new `requests`
   package
6. Run `pc_monitor_agent.py` as normal - CPU temp should now show up on
